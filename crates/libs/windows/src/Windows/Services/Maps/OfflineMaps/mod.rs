...
impl OfflineMapPackage {
    ...
    pub fn StatusChanged<P0>(&self, value: Option<P0>) -> windows_core::Result<i64>
    where
        P0: FnMut(windows_core::Ref<OfflineMapPackage>, windows_core::Ref<windows_core::IInspectable>) -> windows_core::Result<()> + Send + 'static,
    {
        let this = self;
        unsafe {
            let mut result__ = core::mem::zeroed();
            (windows_core::Interface::vtable(this).StatusChanged)(windows_core::Interface::as_raw(this), core::mem::transmute_copy(&value.map(|closure| super::super::super::Foundation::TypedEventHandler::<OfflineMapPackage, windows_core::IInspectable>::new(closure))), &mut result__).map(|| result__)
        }
    }
