...
impl ContactPickerUI {
    ...
    pub fn ContactRemoved<P0>(&self, handler: Option<P0>) -> windows_core::Result<i64>
    where
        P0: FnMut(windows_core::Ref<ContactPickerUI>, windows_core::Ref<ContactRemovedEventArgs>) -> windows_core::Result<()> + Send + 'static,
    {
        let this = self;
        unsafe {
            let mut result__ = core::mem::zeroed();
            (windows_core::Interface::vtable(this).ContactRemoved)(windows_core::Interface::as_raw(this), core::mem::transmute_copy(&handler.map(|closure| super::super::super::Foundation::TypedEventHandler::<ContactPickerUI, ContactRemovedEventArgs>::new(closure))), &mut result__).map(|| result__)
        }
    }
