...
impl IGpioPinProvider {
    ...
    where
        P0: FnMut(windows_core::Ref<IGpioPinProvider>, windows_core::Ref<GpioPinProviderValueChangedEventArgs>) -> windows_core::Result<()> + Send + 'static,
    {
        let this = self;
        unsafe {
            let mut result__ = core::mem::zeroed();
            (windows_core::Interface::vtable(this).ValueChanged)(windows_core::Interface::as_raw(this), core::mem::transmute_copy(&handler.map(|closure| super::super::super::Foundation::TypedEventHandler::<IGpioPinProvider, GpioPinProviderValueChangedEventArgs>::new(closure))), &mut result__).map(|| result__)
        }
    }
